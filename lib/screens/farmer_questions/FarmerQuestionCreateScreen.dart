...
class FarmerQuestionCreateScreen extends StatefulWidget {
  const FarmerQuestionCreateScreen({
    Key? key,
  }) : super(key: key);

  @override
  FarmerQuestionCreateScreenState createState() =>
      FarmerQuestionCreateScreenState();
}

class FarmerQuestionCreateScreenState extends State<FarmerQuestionCreateScreen>
  ...
  submit_form() async {
    String question = _fKey.currentState!.fields['question']!.value;
    String category = _fKey.currentState!.fields['category']!.value.toString();
    if (question.isEmpty) {
    ...
    if (question.isEmpty && imagePath.isEmpty && path.isEmpty) {
      Utils.toast2('Please enter question or attach photo or record audio');
      return;
    }
    if (category.isEmpty) {
      Utils.toast2('Please select category');
      return;
    }
    ...
    RespondModel resp = RespondModel(
        await Utils.http_post('farmer-questions-create', formDataMap));
